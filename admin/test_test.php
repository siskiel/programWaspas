...
// tabel penilaian
$calon_komandan = $koneksi->query("SELECT * FROM calon_komandan");
// ambil bobot kriteria 
     while ($row_komandan = mysqli_fetch_array($calon_komandan)) {
     $id_calon_komandan = $row_komandan['id_calon_komandan'];
     $nama_calon = $row_komandan['nama'];
     $nama []= $nama_calon;
     }
//      while ($row_kriteria = mysqli_fetch_array($kriteria)) {
//      $id_calon_komandan = $row_table1['id_calon_komandan'];
...
$ambil = $koneksi->query("SELECT * FROM penilaian  JOIN calon_komandan ON 
        calon_komandan.id_calon_komandan=penilaian.id_calon_komandan
         ...
         group by penilaian.id_kriteria,penilaian.id_calon_komandan
         ");

$penilaian = $koneksi->query("SELECT * FROM penilaian group by id_calon_komandan");
